    ...
    with _db() as c:
        if project.strip():
            rows = c.execute(
                ...
                "WHERE project = ? AND (text LIKE ? OR meta LIKE ?) "
                "ORDER BY id DESC LIMIT ?",
                (project.strip(), q_like, q_like, limit)
            ).fetchall()
        else:
            rows = c.execute(
                "SELECT id, ts, project, kind, text, meta FROM agent_memory "
                "WHERE (text LIKE ? OR meta LIKE ?) "
                "ORDER BY id DESC LIMIT ?",
                (q_like, q_like, limit)
            ).fetchall()
    out = [{
        "id": r[0], "ts": r[1], "project": r[2], "kind": r[3],
        "text": r[4], "meta": (json.loads(r[5]) if r[5] else {})
    } for r in rows]
